...
namespace MatchZy
{
    public partial class MatchZy
    {
        ...
        private void StartAfterKnifeWarmup()
        {
            ...
            if (matchzy_match_simulate.Value)
            {
                AddTimer(3.0f, () => {
                    if (!isSideSelectionPhase) return;
                    sideSelectionMessageTimer?.Kill();
                    sideSelectionMessageTimer = null;
                    bool doStay = new Random().Next(2) == 0;
                    if (doStay)
                    {
        ...
        private void StartLive()
        {
            ...
            // mp_match_end_restart is intentionally NOT set here: CS2 would queue a Changelevel()
            // (empty map) that fires after mp_match_restart_delay and overrides the plugin's own
            // changelevel to the next map in the series. The plugin handles all map changes itself.
        ...
        private void CheckLiveRequired()
        {
            ...
            bool simulateActive = matchzy_match_simulate.Value ||
                (matchConfig.ChangedCvars.TryGetValue("matchzy_match_simulate", out string? simVal) && (simVal == "1" || simVal == "true"));
